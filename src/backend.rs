...
/// --mstack= rejects: it needs managed user namespacing or none. A trusted settings file
/// overrides that for this machine.
pub fn mstack_settings_text(name: &str) -> String {
    format!("# Generated by nspawn for the mstack image {name}; do not edit.\n[Exec]\nPrivateUsers=managed\n")
}

pub fn mstack_settings_path(name: &str) -> PathBuf {
    Path::new(NSPAWN_SETTINGS_DIR).join(format!("{name}.nspawn"))
}

pub fn ensure_mstack_settings(name: &str) -> Result<()> {
    let path = mstack_settings_path(name);
    let wanted = mstack_settings_text(name);
    ...
    fs::create_dir_all(NSPAWN_SETTINGS_DIR)
        .with_context(|| format!("creating {NSPAWN_SETTINGS_DIR}"))?;
    ...
    pub async fn remove(&self, name: &str, backend: BackendChoice) -> Result<()> {
        match backend {
            ...
            BackendChoice::Mstack => {
                ...
                let _ = fs::remove_file(mstack_settings_path(name));
            }
...
mod tests {
    ...
    #[test]
    fn mstack_settings_force_managed_user_namespaces() {
        let text = mstack_settings_text("fedora-44");
        assert!(text.contains("[Exec]\nPrivateUsers=managed\n"));
        assert_eq!(
            mstack_settings_path("fedora-44"),
            PathBuf::from("/etc/systemd/nspawn/fedora-44.nspawn")
        );
    }
