...
pub struct MachineSettings<'a> {
    ...
    pub tuning: &'a Tuning,
    /// The file with --hostname for a booted machine: its systemd sets the hostname
    /// from /etc/hostname at boot, over what systemd-nspawn set.
    pub hostname_file: Option<&'a Path>,
}
...
pub fn render_with(s: &MachineSettings, route: &NamespaceRoute) -> String {
    ...
    match s.mode {
        ...
        Mode::App => {
            ...
            // systemd-nspawn's own resolv.conf handling would bind the host's stub file
            // over the generated one on a read-only root.
            if s.network == Network::Bridge && !s.no_network && s.bridge.is_some() {
                out.push_str("ResolvConf=off\n");
            }
        }
    }
    ...
    let mut files: Vec<String> = s.tuning.files_lines();
    if let Some(hostname) = s.hostname_file {
        files.push(format!("BindReadOnly={}:/etc/hostname", hostname.display()));
    }
    match s.network {
...
/// What the hooks drop-in carries besides the hooks.
pub struct HookSpec<'a> {
    pub restart: Restart,
    pub limits: &'a Limits,
    pub remove_on_exit: bool,
    /// --device: DeviceAllow= for the unit's cgroup.
    pub tuning: &'a Tuning,
}
...
pub fn write_hooks(
    ...
    let text = render_hooks(&hook_command(config)?, name, route, app_argv, spec);
