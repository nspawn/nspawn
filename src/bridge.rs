...
/// each of its bridges (host0 on the first), the addresses, the default route through
/// `gateway` and the --sysctl values.
pub fn create_netns(
    nets: &[(&NetSpec, Ipv4Addr)],
    name: &str,
    gateway: Ipv4Addr,
    ...
    let result = (|| {
        ...
        for (key, value) in sysctls {
            ...
            run(
                "ip",
                &["netns", "exec", &ns, "sysctl", "-q", "-w", &setting],
            )?;
        }
        Ok(())
    })();
...
pub fn prepare_machine(
    ...
    // --dns first, then nspawn.toml's, then the host's.
    let dns = if record.tuning.dns.is_empty() {
        upstream_dns(&config.dns)
    } else {
        record.tuning.dns.clone()
    };
    ...
    fs::write(&path, resolv_conf(&dns, &record.tuning.dns_search))
        .with_context(|| format!("writing {}", path.display()))?;
        ...
        for net in &nets {
            for member in members.get(net.name.as_str()).into_iter().flatten() {
                ...
                if member.name == r.name {
                    ...
                    others.push((IpAddr::V4(member.address), names));
                }
            }
        }
        let gateway = nets[gateway_index(nets.iter().copied())].subnet.gateway();
        // --add-host, after the machines: the first line for a name wins in glibc.
        for extra in &r.tuning.extra_hosts {
            let address: IpAddr = if extra.ip == "host-gateway" {
                IpAddr::V4(gateway)
            } else {
                extra.ip.parse().unwrap_or(IpAddr::V4(gateway))
            };
            others.push((address, vec![extra.host.clone()]));
        }
...
mod tests {
    ...
    fn generated_files() {
        ...
        let hosts = hosts_file(
            ...
            subnet.gateway(),
            &[(IpAddr::V4(Ipv4Addr::new(10, 99, 0, 3)), vec!["db".into()])],
        );
