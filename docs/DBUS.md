...
| `CreateMachine(s source, s name, a{sv} options) -> o` | `create` | options backend, network (s) or networks (as, several bridge networks, the first one primary), aliases (as, NAME or NETWORK=NAME), publish, force, entrypoint, env, volume, label, restart, memory (t, bytes), cpus (d), pids_limit (t), command, registry, ca_cert; a job |
